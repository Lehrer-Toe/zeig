...
if ($page === 'bewerten' && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['form_action'])) {
        ...
        switch ($_POST['form_action']) {
            case 'save_rating':
                ...
                $final_grade = isset($_POST['final_grade']) && $_POST['final_grade'] !== '' ? 
                               (float)str_replace(',', '.', $_POST['final_grade']) : null;
                ...
                if (!empty($category_ratings)) {
                    $stmt = $db->prepare("
                        INSERT INTO rating_categories (rating_id, category_id, points, created_at)
                        VALUES (?, ?, ?, NOW())
                    ");
                    
                    foreach ($category_ratings as $category_id => $points) {
                        if ($points !== '') {
                            $points_float = floatval(str_replace(',', '.', $points));
                            $stmt->execute([$rating_id, (int)$category_id, $points_float]);
                        }
                    }
                }
                
                // Wenn "Speichern und Durchschnitt übernehmen" geklickt wurde
                if (isset($_POST['save_with_average'])) {
                    // Durchschnitt berechnen und als Endnote setzen
                    $stmt = $db->prepare("
                        SELECT rc.points, rtc.weight
                        FROM rating_categories rc
                        JOIN rating_template_categories rtc ON rc.category_id = rtc.id
                        WHERE rc.rating_id = ?
                    ");
                    $stmt->execute([$rating_id]);
                    ...
                    if (!empty($results)) {
                        ...
                        foreach ($results as $row) {
                            $weighted_sum += $row['points'] * $row['weight'];
                        ...
                        if ($total_weight > 0) {
                            ...
                            $stmt = $db->prepare("
                                UPDATE ratings 
                                SET final_grade = ?, is_complete = 1, updated_at = NOW()
                                WHERE id = ?
                            ");
                            $stmt->execute([$average, $rating_id]);
                        }
                    }
                }
            ...
            case 'save_strengths':
                ...
                if (!empty($strength_items)) {
                    $stmt = $db->prepare("
                        INSERT INTO rating_strengths (rating_id, strength_item_id, created_at)
                        VALUES (?, ?, NOW())
                    ");
