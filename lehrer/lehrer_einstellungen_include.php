...
<?php
// Schülerdaten für JavaScript vorbereiten - NUR Schüler die noch in KEINER AKTIVEN Gruppe sind!
$students_by_class = [];
...
if (isset($_GET['debug']) && $_GET['debug'] === '1') {
    ...
    foreach ($students_by_class as $class_id => $students) {
        ...
        foreach ($students as $s) {
            // Prüfen ob dieser Schüler wirklich in keiner aktiven Gruppe ist
            $check = $db->prepare("
            ...
            if ($inGroup) {
                if ($inGroup['is_active']) {
                    echo "  ⚠️ FEHLER: {$s['first_name']} {$s['last_name']} (ID: {$s['id']}) ist bereits in aktiver Gruppe '{$inGroup['name']}' (ID: {$inGroup['id']})\n";
                } else {
                    echo "  ℹ️ INFO: {$s['first_name']} {$s['last_name']} (ID: {$s['id']}) ist in inaktiver Gruppe '{$inGroup['name']}' (ID: {$inGroup['id']})\n";
                }
            } else {
